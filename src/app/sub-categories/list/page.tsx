...
import CategoriesForm from '../form/SubCategory';
...
const SubCategories = () => {
  ...
  const getSubCategories = () => {
    setLoading(true);
    apiService.get(`categories/sub`).then((response) => {
    ...
    <DefaultLayout>
      <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className='flex justify-between mb-7'>
            ...
            <section className="">
              <div className='mb-6 text-xl font-semibold text-black dark:text-white'>Sub Categories</div>
              <h6 className='text-black dark:text-white'>Available departments</h6>
            </section>
            <div className='h-10 gap-3  flex'>
              <Button label="Add New" outlined severity="info" size='small' icon="pi pi-plus" onClick={handleAddSubCategories} title='Add a new department' />
        ...
        <div className="flex flex-col">
            <DataTable value={departments} dataKey="id" size='normal' emptyMessage="No Sub categories found." resizableColumns paginator
              rows={5} rowsPerPageOptions={[5, 10, 25, 50]}
            >
              <Column field="name" header="Sub Category" sortable filter />
              <Column field="parentID" header="Category Id" sortable filter />
